...
public abstract class BaseCtl extends HttpServlet {
	...
	protected BaseBean populateDTO(BaseBean dto, HttpServletRequest request) {
		System.out.println("BaseCtl BaseBean populateDTO Method");
		String createdBy = request.getParameter("createdBy");
		String modifiedBy = null;

		UserBean userbean = (UserBean) request.getSession().getAttribute("user");
       System.out.println("BaseCtl UserBean"+userbean);
		if (userbean == null) {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("BaseCtl Service Mathod started");
		...
		if (DataValidator.isNotNull(op) && !OP_CANCEL.equalsIgnoreCase(op) && !OP_VIEW.equalsIgnoreCase(op)
			...
			if (!validate(request)) {
				...
				System.out.println("Inside BaseCtl Validate Mathod");
				ServletUtility.setBean(bean, request);
				ServletUtility.forward(getView(), request, response);
				return;
			}
		}
		System.out.println(" BaseCtl Service success");
